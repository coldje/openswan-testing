...
setup_host_make() {
    ...
    echo "$hostroot/sbin/init : ${BASICROOT}/sbin/init ${host}/root"
    ...
    echo "$TAB -(cd ${BASICROOT} && find . -print | cpio -pld $POOLSPACE/$hostroot 2>&1 | grep -v newer )"
    echo "$TAB rm -rf $hostroot/var"

    echo "$TAB (cd ${BASICROOT} && find var -print | cpio -pd $POOLSPACE/$hostroot 2>&1 | grep -v newer )"
